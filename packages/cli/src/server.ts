  ...
  const server = createServer(
    async (req: IncomingMessage, res: ServerResponse) => {
      try {
        ...
        // Uses pre-computed cache file if available (avoids slow git on Docker bind mounts)
        if (pathname === '/api/diff/raw') {
          const ref = url.searchParams.get('ref') || 'work';
          const whitespace = url.searchParams.get('whitespace');
          const cacheFile = join(process.cwd(), '.diffity-cached-diff.txt');
          let raw: string;
          try {
            // Prefer cached diff (generated by host for speed)
