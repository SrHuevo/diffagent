...
2. Check that a review session exists: run `{{binary}} agent list`. If this fails with "No active review session", tell the user to start diffagent first (e.g. `{{binary}}` or **{{slash}}diff**).
